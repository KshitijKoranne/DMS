...
import { dmsUsers, roleDescriptions } from "@/lib/users";
...
const modules: Array<[Module, LucideIcon, string]> = [
  ["Dashboard", LayoutDashboard, "/"],
  ["Register", FileText, "/documents"],
  ["Workflow", Workflow, "/workflow"],
  ["E-Sign", Fingerprint, "/signatures"],
  ["Training", BookOpenCheck, "/training"],
  ["Retention", FileArchive, "/retention"],
  ["Reports", Activity, "/reports"],
  ["Admin", Settings2, "/admin"],
];
...
function documentsForRole(documents: DocumentRecord[], userName: string, role: Role) {
  if (role === "System Admin") return documents;
  if (role === "QA Approver") {
    return documents.filter((document) =>
      ["In Review", "Approved", "Effective", "Under Revision", "Retired"].includes(document.status),
    );
  }
  if (role === "Technical Reviewer") {
    return documents.filter(
      (document) =>
        ...
        document.reviewers.some((reviewer) => reviewer.toLowerCase().includes("reviewer")),
    );
  }
  if (role === "Training Coordinator") {
    return documents.filter((document) => document.training !== "Not required");
  }
  return documents.filter((document) => document.owner === userName);
}
...
export function DmsClient({ activeModule }: { activeModule: Module }) {
  const pathname = usePathname();
  const [documents, setDocuments] = useState(documentsSeed);
  const [selectedId, setSelectedId] = useState(documentsSeed[0].id);
  const [currentUserId, setCurrentUserId] = useState(dmsUsers[0].id);
  ...
  const [signature, setSignature] = useState({
    signer: dmsUsers[0].name,
    role: dmsUsers[0].role as Role,
  ...
  useEffect(() => {
    ...
    async function loadDocuments() {
      try {
        ...
        const payload = (await response.json()) as DmsPayload;
        if (cancelled) return;
        ...
        setSelectedId((current) =>
          payload.documents.some((document) => document.id === current)
            ? current
            : payload.documents[0]?.id ?? documentsSeed[0].id,
        );
      } catch {
        return;
      }
  ...
  const currentUser = dmsUsers.find((user) => user.id === currentUserId) ?? dmsUsers[0];
  const visibleDocuments = useMemo(
    () => documentsForRole(documents, currentUser.name, currentUser.role),
    [documents, currentUser.name, currentUser.role],
  );
  const visibleFilteredDocuments = useMemo(
    () => filteredDocuments.filter((document) => visibleDocuments.some((item) => item.id === document.id)),
    [filteredDocuments, visibleDocuments],
  );
  const visibleSelectedDocument =
    visibleDocuments.find((document) => document.id === selectedDocument.id) ??
    visibleDocuments[0] ??
    selectedDocument;
  ...
  const metrics = [
    {
      ...
      value: visibleDocuments.length.toString(),
      detail: currentUser.role,
      icon: FileText,
    },
    {
      label: currentUser.role === "Training Coordinator" ? "Open training" : "Open signatures",
      value:
        currentUser.role === "Training Coordinator"
          ? documents.filter((document) => document.training === "Pending").length.toString()
          : pendingSignatures.length.toString(),
  ...
  function applyPayload(payload: DmsPayload, preferredId?: string) {
    setDocuments(payload.documents);
    setAudit(payload.audit);
    setSelectedId((current) =>
      payload.documents.some((document) => document.id === (preferredId ?? current))
        ? preferredId ?? current
        : payload.documents[0]?.id ?? documentsSeed[0].id,
    );
  }

  async function advanceDocument(document: DocumentRecord) {
    ...
    try {
      ...
      applyPayload((await response.json()) as DmsPayload, document.id);
      return;
    } catch {}
  ...
  async function createDocument() {
    ...
    try {
      ...
      if (!response.ok) throw new Error("Document create failed.");
      const payload = (await response.json()) as DmsPayload;
      const created = payload.documents.find(
        (document) => document.title === newDoc.title.trim(),
      );
      applyPayload(payload, created?.id);
      setNewDoc({
        title: "",
        type: "SOP",
        department: "Quality Assurance",
  ...
  async function signDocument() {
    ...
    const exists = visibleSelectedDocument.signatures.some(
      (item) => item.meaning === signature.meaning,
    );
    if (exists) return;

    try {
      const response = await fetch(`/api/documents/${visibleSelectedDocument.id}/signatures`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      applyPayload((await response.json()) as DmsPayload, visibleSelectedDocument.id);
      setSignature((current) => ({ ...current, password: "", reason: "" }));
      return;
    } catch {}

    const raw = `${visibleSelectedDocument.id}:${signature.signer}:${signature.meaning}:${signature.reason}:${Date.now()}`;
    const signed: SignatureRecord = {
      id: `SIG-${2300 + audit.length}`,
      documentId: visibleSelectedDocument.id,
    ...
    setDocuments((current) =>
      current.map((document) =>
        document.id === visibleSelectedDocument.id
          ? {
              ...document,
  ...
  const selectedAudit = audit.filter(
    (entry) => entry.documentId === visibleSelectedDocument.id,
  );
        ...
        <div className="mx-auto flex w-full max-w-[1540px] flex-col gap-5 px-4 py-5 sm:px-6 lg:px-8">
          <header className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <label className="flex h-10 items-center gap-2 rounded-md border border-[#d8dfd6] bg-white px-3 text-sm font-medium text-[#314039] shadow-sm">
                <UserCog size={16} />
                <select
                  className="bg-transparent outline-none"
                  onChange={(event) => {
                    const nextUser = dmsUsers.find((user) => user.id === event.target.value);
                    setCurrentUserId(event.target.value);
                    if (nextUser) {
                      setSignature((current) => ({
                        ...current,
                        signer: nextUser.name,
                        role: nextUser.role as Role,
                      }));
                    }
                  }}
                  value={currentUserId}
              ...
              <Link
                className="inline-flex h-10 items-center gap-2 rounded-md border border-[#d8dfd6] bg-white px-3 text-sm font-medium text-[#314039] shadow-sm transition hover:bg-[#f2f5ef]"
                href="/signatures"
              >
                <Bell size={16} />
      ...
      <section className="mx-auto grid w-full max-w-[1540px] gap-5 px-4 py-5 sm:px-6 lg:grid-cols-[260px_minmax(0,1fr)] lg:px-8">
        <aside className="h-fit rounded-lg border border-[#dfe5dc] bg-white p-3 shadow-sm">
          ...
          <div className="mt-4 rounded-md border border-[#e1e7de] p-3">
            <p className="text-sm font-semibold text-[#21372f]">{currentUser.name}</p>
            <p className="mt-1 text-sm text-[#66736b]">{currentUser.role}</p>
            <p className="mt-1 text-xs text-[#7a867e]">{currentUser.department}</p>
          </div>
        </aside>
        ...
        <div className="grid gap-5">
          ...
          {activeModule === "Register" && (
            <Register
              documents={visibleFilteredDocuments}
              selectedDocument={visibleSelectedDocument}
              query={query}
              setQuery={setQuery}
          ...
          {activeModule === "Training" && (
            <TrainingCenter documents={visibleDocuments} training={allTraining.filter(({ document }) => visibleDocuments.some((item) => item.id === document.id))} />
          )}

          {activeModule === "Retention" && (
            <Retention documents={visibleDocuments} />
          )}

          {activeModule === "Reports" && (
            <Reports documents={visibleDocuments} audit={audit} />
          )}

          {activeModule === "Admin" && <Admin currentUser={currentUser} />}
        </div>
      </section>
    </main>
  );
...
function Dashboard({
  ...
  currentUser: (typeof dmsUsers)[number];
}) {
    ...
    <section className="grid gap-5">
      <div className="grid gap-5 2xl:grid-cols-[minmax(0,1fr)_380px]">
        <Panel
          ...
          subtitle={`Focused view for ${currentUser.name} in ${currentUser.department}.`}
...
function Admin({ currentUser }: { currentUser: (typeof dmsUsers)[number] }) {
  return (
    <section className="grid gap-5 2xl:grid-cols-2">
      <Panel title="System configuration" subtitle={`Configuration access for ${currentUser.role}.`} icon={Settings2}>
      ...
      <Panel title="Roles and permissions" subtitle="Baseline GxP roles for controlled access." icon={UserCog}>
        <div className="grid gap-3">
          {(Object.entries(roleDescriptions) as Array<[Role, string]>).map(([role, scope]) => (
      ...
      <Panel title="Users" subtitle="Seed users for access-matrix and role testing." icon={UserRoundCheck}>
        <div className="overflow-x-auto">
          <table className="w-full min-w-[720px] border-separate border-spacing-0 text-left text-sm">
            <thead>
              <tr className="text-[#69756d]">
                {["User", "Role", "Department", "Email"].map((heading) => (
                  <th className="border-b border-[#e7ece4] px-4 py-3 font-medium" key={heading}>{heading}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {dmsUsers.map((user) => (
                <tr key={user.id}>
                  <td className="border-b border-[#edf1ea] px-4 py-3 font-semibold text-[#24332d]">{user.name}</td>
                  <td className="border-b border-[#edf1ea] px-4 py-3 text-[#405047]">{user.role}</td>
                  <td className="border-b border-[#edf1ea] px-4 py-3 text-[#405047]">{user.department}</td>
                  <td className="border-b border-[#edf1ea] px-4 py-3 text-[#69756d]">{user.email}</td>
                </tr>
              ))}
            </tbody>
          </table>
